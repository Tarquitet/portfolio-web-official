document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- REEMPLAZA LA SECCIÓN DE IDIOMAS Y SKILLS CON ESTO ---

  // 3. Idiomas (Etiquetas horizontales)
  const langContainer = document.getElementById('cv-languages-list');
  if (langContainer && d.languages) {
    langContainer.innerHTML = d.languages
      .map((lang) => {
        const label = lang.level || (lang.score >= 130 ? 'B1 Level' : '');
        return `<span class="skill-tag-block"><b>${lang.name}</b> (${label})</span>`;
      })
      .join('');
  }

  // 4. Skills (Etiquetas horizontales)
  const renderAsText = (items, containerId) => {
    ...
    container.innerHTML = items
      .map((skill) => {
        ...
        return `<span class="skill-tag-block">${skillName}</span>`;
      })
      .join('');
  };
