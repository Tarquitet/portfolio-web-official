import{CONFIG}from './config.min.js';export class Renderer{constructor(data,gallery){this.data=data;this.gallery=gallery;this.cachedVideos=null;this.template=document.getElementById('uni-card-template');}renderAll(){this.renderStructure();this.renderMenu();this.renderDev();this.renderVideo();this.renderDesign();this.renderGallery();this.renderDatabase();this.renderProfile();this.renderFooter();if(window.Utils)window.Utils.initIcons();}_resolveImgPath(item,type){if(type==='VIDEO'){const videoId=item.id||this._extractYoutubeId(item.link);return videoId?`https://i.ytimg.com/vi_webp/${videoId}/maxresdefault.webp`:window.Utils.getSmartPath(file,'VIDEO');}const file=item.img||item.fileName||item.image;if(!file)return '';const cat=type==='ART'?'ART':type==='DESIGN'?'DESIGN':'DEV';return window.Utils.getSmartPath(file,cat);}_extractYoutubeId(url){if(!url)return null;const regExp=/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]*).*/;const match=url.match(regExp);return match&&match[2].length===11?match[2]:null;}_zigZagSort(projects,limit,isArt=false){const groups={};projects.forEach((p)=>{const arr=isArt?p.tags:p.tools;const key=arr&&arr.length>0?arr[0]:'Otros';if(!groups[key])groups[key]=[];groups[key].push(p);});const result=[];const keys=Object.keys(groups);let index=0;let keepAdding=true;while(result.length<limit&&keepAdding){keepAdding=false;for(const key of keys){if(result.length>=limit)break;if(groups[key][index]){result.push(groups[key][index]);keepAdding=true;}}index++;}return result;}async _fetchGithubRepos(username='tarquitet'){try{const res=await fetch(`https://api.github.com/users/${username}/repos?sort=updated&per_page=6`);const repos=await res.json();const githubProjects=await Promise.all(repos.map(async(repo)=>{let firstImage=null;try{const readmeRes=await fetch(`https://raw.githubusercontent.com/${username}/${repo.name}/main/README.md`);if(readmeRes.ok){const readmeText=await readmeRes.text();const regex=/!\[.*?\]\((.*?)\)|<img.*?src=["'](.*?)["']/;
